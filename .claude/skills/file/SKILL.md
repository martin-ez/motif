...
`add` refuses it and lists the open chain heads. An epic takes `--blocked-by`
instead, naming the epic it comes off — `add` refuses that too, marking the open
ends of the chain. Only the first epic in an empty tracker comes off nothing.
