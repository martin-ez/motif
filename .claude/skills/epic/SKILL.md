...
Then read the code each one touches, and group them: which file, which module,
which invariant.
...
the same function, and no edge will ever say so. Ready children that reach for
the same file belong in different batches whatever the graph says.
...
Three is the default because the gate is sized for it. `scripts/check-mutants.sh`
takes a quarter of the machine's cores, so three sweeps at once still leave the
machine headroom — and a sweep without headroom reports timeouts as survivors,
...
time. Per plan: test-first, tests in `tests/`, the invariant named, scope inside
its own issue. Across the set: the same file twice, the same helper invented
twice, one invariant held two different ways, a plan assuming another's unmerged
work.
...
They run at once, and the sweep is the check that shares the machine. An agent
reporting a timeout rather than a survivor is reporting load: it lowers
`MOTIF_MUTANTS_JOBS` and runs it again rather than writing a test for it.
